...
absl::StatusOr<Shape> GetShape(Shape shard_shape, int64_t num_shards) {
...
absl::StatusOr<ArraySpec> CreateArraySpec(Client* client,
  ...
  TF_ASSIGN_OR_RETURN(Shape shape,
                      GetShape(shard_shape, device_indices.size()));
...
absl::StatusOr<tsl::RCReference<Array>> CreateArray(
  ...
  TF_ASSIGN_OR_RETURN(Shape shape, GetShape(shard_shape, base_values.size()));
...
void AssertArrayContent(Client* client, Array* array,
  ...
  TF_ASSERT_OK_AND_ASSIGN(Shape expected_shape,
                          GetShape(expected_shard_shape, base_values.size()));
